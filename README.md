...
DART(전자공시시스템) 최신 공시와 네이버 뉴스를 병렬로 폴링해 호재성 이슈를 골라 Discord/Webex로 알림을 보내는 봇.
...
```
NewsPollerService (별도 스레드 1개에서 두 주기 운용)
  ├─ RssClient.fetch()              언론사 속보 RSS 6개 피드 (30초 주기, 기본)
  └─ NaverNewsClient.search()       키워드별 네이버 뉴스 검색 (120초 주기, 키 설정 시)
...
| `NEWS_RSS_FEEDS` | 검증된 6개 피드 | 언론사 RSS (`이름\|URL` 콤마 구분) |
